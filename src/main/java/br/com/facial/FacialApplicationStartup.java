...
import br.com.facial.recognitionconfig.RecognitionConfig;
import br.com.facial.recognitionconfig.RecognitionConfigRepo;
...
public class FacialApplicationStartup implements ApplicationListener<ApplicationReadyEvent> {
	...
	private void createConfiguration() {
		RecognitionConfig config = recognitionConfigRepo.findByDescription("status");
		if (config == null) {
			RecognitionConfig recognitionConfig = new RecognitionConfig();

			recognitionConfig.setCodeStatus("0");
			recognitionConfig.setDescription("status");

			recognitionConfigRepo.save(recognitionConfig);
		}
	...
	private void createPerson() {
		...
		if (person == null) {
			personRepo.save(Person.builder().name("Joao").lastName("da Silva").email("joao.silva@gmail.com")
					.phone("47 1234-4567").build());
		}
